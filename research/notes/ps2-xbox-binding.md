...
## Part 1: call-graph crawl — 791 PS2 methods across 254 classes, address-verified
...
The eighty-sixth pass crawled one generic CGt2dFrameHierarchy synchronization method; one fresh entry was retained after deduplication.
...
The eightieth pass crawled five CGTFileSystem file-position, read, initialization, and default-filesystem methods and reached CGtUnicode::PrintReal. Six fresh entries were retained after deduplication.

The seventy-ninth pass crawled four CGtUnicode string/number formatting and copy methods. No additional named callees were reached; all four anchors were independently verified.
...
`CB3World__Prepare` at Xbox atlas address `0x001aa100` resolves to function start `0x001aa040`, whose body is a short float/render-state routine; it does not match the large PS2 `CB3World::Prepare` linker-map method. This label is rejected pending a real cross-architecture match.
`CB3World__Release` at atlas `0x001aa990` resolves to Xbox function start `0x001aa6f0`, a large float-heavy routine; it does not match the PS2 `CB3World::Release` lifecycle method. This common label is also rejected.
`CB3DebugManager::UpdateStartOfFrame()` was directly compared after a valid PS2 anchor was found at `0x00130430` (map offset `0x100`) and Xbox atlas address `0x00013f10` (actual start `0x00013d60`). PS2 decompilation is a short four-call wrapper (`+0x10`, `+0x710`, `+0x4210`, then a global update), while the Xbox body is a large float/performance-render routine. The same-name label is rejected.
`CB3DebugManager::RenderPerfMonDataCallback` also failed direct comparison: PS2 anchor `0x0012f980` (map offset `0xc0`) decompiles to a truncated 84-byte MIPS body, while Xbox atlas `0x00014470` resolves to `0x00014450`, a 146-byte callback that allocates state and calls a render/performance helper with constants. No structural confirmation was made.
`CB3GraphicsManager::OpenRenderWare()` was directly compared at PS2 `0x001eb550` (map offset `0`) and Xbox atlas `0x0003d690` (actual start `0x0003d680`). PS2 iterates viewport/render-mode slots and inspects short fields; Xbox checks one manager field, creates a resource, and calls a render setup helper with constants. The same-name binding is rejected.
`CB3GraphicsManager::OpenViewport` was checked at PS2 `0x001eb0b0` (offset `0`) versus Xbox atlas `0x0003d9e0` (actual start `0x0003d730`). The PS2 body is a short viewport-slot loop (92 bytes), while Xbox is a 1017-byte render setup routine with float and pointer-heavy logic; no binding.
`CB3GraphicsManager::Construct()` was checked at PS2 `0x001eb800` (map offset `0xc0`) versus Xbox atlas `0x0003c8a0` (actual start `0x0003c860`). PS2 is a short two-slot readiness scan; Xbox is a 387-byte setup routine with float/vector state construction. No binding.
